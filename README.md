...
| <a name="input_dhcp"></a> [dhcp](#input\_dhcp) | Configurations for DHCP options for this VPC | <pre>object({<br>    domain_name          = optional(string)<br>    domain_name_servers  = optional(list(string), ["AmazonProvidedDNS"])<br>    ntp_servers          = optional(list(string))<br>    netbios_name_servers = optional(list(string))<br>    netbios_node_type    = optional(number)<br>    tags                 = optional(map(string))<br>  })</pre> | `{}` | no |
...
| <a name="output_nacls"></a> [nacls](#output\_nacls) | Network ACLs created for subnet groups in this VPC |
| <a name="output_nacls_by_group"></a> [nacls\_by\_group](#output\_nacls\_by\_group) | Network ACLs created for subnet groups in this VPC, nested by group (ex. module.my\_vpc.nacls\_by\_group["my-group"].arn) |
...
| <a name="output_route_tables_by_group"></a> [route\_tables\_by\_group](#output\_route\_tables\_by\_group) | Route tables created for this VPC. Nested by group and AZ (ex. module.my\_vpc.route\_tables\_by\_group[\"my-group\"][\"us-west-1a\"].arn) for private subnet groups,<br>and nested by group (ex. module.my\_vpc.route\_tables\_by\_group[\"my-group\"].arn) for public and airgapped subnet groups |
...
| <a name="output_subnets"></a> [subnets](#output\_subnets) | Subnets created in this VPC |
